...
func parseQueryNamedVal[P h.HasID](query, pResType string, queryParam QueryType, pLookup map[string]P) (int32, error) {
	id, err := checkQueryIDVal(query, queryParam, len(pLookup))
	if err == nil {
		return id, nil
	}
