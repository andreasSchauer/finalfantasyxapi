...
func filterShopsEquipment(cfg *Config, r *http.Request) ([]int32, error) {
	...
	if err != nil {
		return nil, newHTTPError(http.StatusInternalServerError, fmt.Sprintf("couldn't filter %ss by auto-ability id '%s', character id '%s', empty slots amount '%s'.", i.resourceType, h.PtrToString(autoAbilityID), h.PtrToString(charID), h.FormatIntSlice(emptySlots)), err)
	}
