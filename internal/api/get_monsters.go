...
func (cfg *Config) getMonster(r *http.Request, i handlerInput[seeding.Monster, Monster, NamedAPIResource, NamedApiResourceList], id int32) (Monster, error) {
	...
	response := Monster{
		...
		Category:             newNamedAPIResourceFromEnum(cfg, cfg.e.monsterCategory.endpoint, monster.Category, cfg.t.MonsterCategory),
		...
		BaseStats:            namesToNamedResourceAmounts(cfg, cfg.e.stats, monster.BaseStats, newBaseStat),
		...
		StatusResists:        namesToNamedResourceAmounts(cfg, cfg.e.statusConditions, monster.StatusResists, newStatusResist),
