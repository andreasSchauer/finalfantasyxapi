...
func (cfg *Config) initMonstersParams() {
	...
	params := map[string]QueryType{
		...
		"status-resists": {
			...
			Usage:       "?status-resists={status_condition_id},{status_condition_id},...",
			ExampleUses: []string{"status-resists=1,4"},
			ForList:     true,
			ForSingle:   false,
			ForSections: []string{"monsters"},
		...
		"resistance": {
			ID:              6,
			Description:     "Specifies the minimum resistance for the status-resists parameter. Resistance is an integer ranging from 1 to 254 (immune). The value 'immune' can also be used, which counts as 254.",
			Usage:           "status-resists={status_condition_id},{status_condition_id},...&resistance={1-254 or 'immune'}",
			ExampleUses:     []string{"status-resists=13&resistance=50", "status-resists=4,17&resistance=30", "status-resists=sleep&resistance=immune"},
			ForList:         true,
			ForSingle:       false,
			ForSections:     []string{"monsters"},
			RequiredParams:  []string{"status-resists"},
		...
		"auto-abilities": {
			...
			Usage:       "?auto-abilities={auto_ability_id},{auto_ability_id},...",
			ExampleUses: []string{"?auto-abilities=16", "?auto-abilities=99,100"},
			ForList:     true,
			ForSingle:   false,
			ForSections: []string{"monsters"},
		...
		"ronso-rage": {
			...
			Usage:       "?ronso-rage={ronso_rage_id}",
			ExampleUses: []string{"?ronso-rage=5"},
			ForList:     true,
			ForSingle:   false,
			ForSections: []string{"monsters"},
